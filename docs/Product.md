...
- **캐싱(Database Fallback)**
    - 최신 데이터를 받아올 경우 `node:sqlite` DB에 데이터를 저장 (date를 key로 사용).
    - Rate Limit, 인증 오류 발생, 타 API 이상 등 500 에러 발생 시 SQLite DB에서 마지막에 캐시된 최신 데이터를 불러와 반환.
...
### 2-1. 강제 DB 적재 (fitop-db)

| 항목       | 설명                                                                                              |
| ---------- | ------------------------------------------------------------------------------------------------- |
| 엔드포인트 | `GET /api/fitop-db`                                                                               |
| 입력       | 없음                                                                                              |
| 출력       | 외국인/기관 매매 상위 10개 종목 처리결과 (DB Update)                                              |
| 목적       | 수동 혹은 스케줄러로 호출되어 KIS API에서 최신 데이터를 가져와 무조건 DB의 데이터를 업데이트한다. |
...
| 출력       | 종목 관련 뉴스를 일별로 검색/취합 후 중복 제거한 목록 (일 최대 5건 제어) |
